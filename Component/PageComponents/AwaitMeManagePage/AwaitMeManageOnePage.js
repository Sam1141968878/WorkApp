...
export default class AwaitMeManageOnePage extends PureComponent{
  state={
    AwaitMeManageOnePageData:[],
    NullState:false,
  }
  ...
  componentDidMount() {
      fetchPost(`http://${this.UserInformationData[0].serverParameters}/workflow`,{
        ...
        this.setState({
            AwaitMeManageOnePageData:res.data.items
        })
      )
  ...
  render() {
    return (
      ...
      >
                {
                    this.state.AwaitMeManageOnePageData.length!=0
                    ?
                        <View>
                          {
                              this.state.AwaitMeManageOnePageData.map((item,index)=>
